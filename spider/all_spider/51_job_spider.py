...
def log(meg: str, *args):
    '''日志函数'''
    print(meg, args)
    with open('51job_log.txt', 'a') as fp:
        fp.write(str(datetime.now()) + ':' + meg + str(args) + '\n')


def save_to_mysql(data: dict, cursor) -> None:
    sql = """
    insert into bs_one (web, key_type, kw, job_name, company, city, salary, exp, edu) values (
        %s, %s, %s, %s, %s, %s, %s, %s, %s
    )
    """
...
def parse_every_page(url: str, key_type: str, kw: str) -> list:
    ...
    for page in range(1, page_num + 1):
        ...
        data_list += parse_url(url, key_type, kw)
...
if __name__ == '__main__':
    ...
    log('start spider')
    for url, kw, kw_type in gen_url_list():
        log('start :', url)
        data_list += parse_every_page(url, kw_type, kw)
        time.sleep(600)

    log('start save')
    gevent.joinall([gevent.spawn(save_to_mysql, data, cursor)
